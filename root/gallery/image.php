...
*/
if (($image_data['image_user_id'] != $user->data['user_id']) && ($image_data['image_status'] == phpbb_ext_gallery_core_image::STATUS_ORPHAN))
{
	//trigger_error('NOT_AUTHORISED');
	$image_error = 'not_authorised.jpg';
}

if ((!$phpbb_ext_gallery->auth->acl_check('i_view', $album_id, $album_data['album_user_id'])) || (!$phpbb_ext_gallery->auth->acl_check('m_status', $album_id, $album_data['album_user_id']) && ($image_data['image_status'] == phpbb_ext_gallery_core_image::STATUS_UNAPPROVED)))
{
	//trigger_error('NOT_AUTHORISED');
	$image_error = 'not_authorised.jpg';
}
...
* Hotlink prevention
*/
if (!$phpbb_ext_gallery->config->get('allow_hotlinking') && isset($_SERVER['HTTP_REFERER']))
{
	$good_referers = array($config['server_name']);
	if ($phpbb_ext_gallery->config->get('hotlinking_domains') != '')
	{
		$good_referers = array_merge($good_referers, explode(',', $phpbb_ext_gallery->config->get('hotlinking_domains')));
	}
...
$image_tools = new phpbb_ext_gallery_core_file($phpbb_ext_gallery->config->get('gdlib_version'));
...
// Watermark
if (false && $phpbb_ext_gallery->config->get('watermark_enabled') && $album_data['album_watermark'] && !$phpbb_ext_gallery->auth->acl_check('i_watermark', $album_id, $album_data['album_user_id']) && $possible_watermark)
{
	$filesize_var = '';
	$image_tools->set_last_modified(@filemtime($phpbb_ext_gallery->url->path('phpbb') . $phpbb_ext_gallery->config->get('watermark_source')));
	$image_tools->watermark_image($phpbb_ext_gallery->url->path('phpbb') . $phpbb_ext_gallery->config->get('watermark_source'), $phpbb_ext_gallery->config->get('watermark_position'), $phpbb_ext_gallery->config->get('watermark_height'), $phpbb_ext_gallery->config->get('watermark_width'));
}
